...
try {
  ...
  const publication = await serviceRpc("static_weekly_v2_publish_draft", `${quote(draft.data.version_id)},1,1,${quote(manager.managerId)},${quote(manager.managerName)},'base-publish','publish',null`);
  assert.equal(publication.revision, 2, "revision-1 draft publishes directly through service_role");
  ...
  const futureDraftInput = createStaticWeeklyDraftRpcInput({ result: futureCompiled, expectedRevision: 2, actor: { ...manager, idempotencyKey: "future-create" } });
  const futureDraft = await serviceRpc("static_weekly_v2_create_draft", `${quote(futureDraftInput.effectiveStart)},${quote(futureDraftInput.objectiveVersion)},${json(futureDraftInput.objective)},${json(futureDraftInput.inputProvenance)},${json(futureDraftInput.document)},2,${quote(manager.managerId)},${quote(manager.managerName)},'future-create'`);
  const update = await serviceRpc("static_weekly_v2_update_draft", `${quote(futureDraft.data.version_id)},${json(futureDraftInput.document)},${json(futureDraftInput.objective)},${json(futureDraftInput.inputProvenance)},1,3,${quote(manager.managerId)},${quote(manager.managerName)},'future-update'`);
  assert.equal(update.revision, 4);

  const pto = await serviceRpc("static_weekly_v2_apply_exception", `'pto','2026-10-06',null,null,${quote(publication.data.version_id)},${quote(publication.data.publication_id)},'approved seven-day PTO',${json({ slotId: source.slots[0].id })},4,${quote(manager.managerId)},${quote(manager.managerName)},'pto-a',null`);
  const absence = await serviceRpc("static_weekly_v2_apply_exception", `'daily_absence','2026-10-06',null,null,${quote(publication.data.version_id)},${quote(publication.data.publication_id)},'approved adjacent daily absence',${json({ slotId: source.slots[1].id })},5,${quote(manager.managerId)},${quote(manager.managerName)},'absence-b',null`);
  assert.equal(absence.revision, 6);
  await expectFailure(`set role service_role; select public.static_weekly_v2_apply_exception('pto','2026-10-06','10:00','11:00',${quote(publication.data.version_id)},${quote(publication.data.publication_id)},'changed time and reason',${json({ slotId: source.slots[0].id })},4,${quote(manager.managerId)},${quote(manager.managerName)},'pto-a',null)`, /idempotency key/i);
  ...
  assert.equal(accepted.length, 2, "the accepted exception identity covers the complete adjacent-date weekly set");
  ...
  const projectionInput = createStaticWeeklyProjectionRpcInput({ result: overlayCompiled, publicationId: publication.data.publication_id, expectedRevision: 6, actor: { ...manager, idempotencyKey: "weekly-projection" } });
  ...
  assert.equal(await scalar(`select count(*) from public.weekly_schedule_occurrences where projection_id=${quote(projection.data.projection_id)} and state='review'`), "1", "required review is retained as a projection result rather than caller-authored data");
  await expectFailure(`set role service_role; select public.static_weekly_v2_materialize_projection(${quote(projectionInput.publicationId)},${quote(projectionInput.serviceDate)},${quote(projectionInput.exceptionSetDigest)},'forged-compiler',${json(projectionInput.objective)},${json(projectionInput.metrics)},${quote(projectionInput.replayDigest)},${json(projectionInput.envelope)},6,${quote(manager.managerId)},${quote(manager.managerName)},'forged-projection')`, /compiler|identity|projection/i);

  const reversal = await serviceRpc("static_weekly_v2_apply_exception", `'reverse','2026-10-06',null,null,${quote(publication.data.version_id)},${quote(publication.data.publication_id)},'PTO cancelled',${json({ reversesExceptionId: pto.data.exception_id })},7,${quote(manager.managerId)},${quote(manager.managerName)},'reverse-pto',${quote(pto.data.exception_id)}`);
  assert.equal(reversal.revision, 8);
  const reversalArgs = `'reverse','2026-10-06',null,null,${quote(publication.data.version_id)},${quote(publication.data.publication_id)},'PTO cancelled',${json({ reversesExceptionId: pto.data.exception_id })},7,${quote(manager.managerId)},${quote(manager.managerName)},'reverse-pto',${quote(pto.data.exception_id)}`;
  ...
  const replacement = await serviceRpc("static_weekly_v2_replace_incumbency", `${quote(source.slots[1].id)},'30000000-0000-4000-8000-000000000006','Jordan Later','2026-10-19',8,${quote(manager.managerId)},${quote(manager.managerName)},'replace-incumbency'`);
  assert.equal(replacement.operation, "replace_incumbency");
  assert.equal(await scalar(`select operation from public.weekly_schedule_authority_revisions where authority_revision=9`), "replace_incumbency");
