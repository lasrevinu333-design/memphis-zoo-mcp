...
const exactPendingReleaseMigrations = [
  alignTwoInternalAbsencesFile,
  pauseGuestNotificationsWhenDisabledFile,
...
assert.match(releaseInput.cutover.phase_order[3], /four exact ordered pending migrations.*two-internal-absence alignment.*pausing guest notifications while disabled.*event-reminder workday\/current-owner contract.*current static-weekly event-reminder authority.*ledger to advance exactly four entries.*do not replay historical production migrations/i);
...
assert.equal(productionMigrationState.observed_production.public_function_count, 503);
...
assert.equal(productionMigrationState.target.source_migration_file, eventReminderStaticWeeklyAuthorityFile);
...
assert.equal(releaseEvidence.migrations.at(-1).name, eventReminderStaticWeeklyAuthorityFile);
...
assert.equal(releaseEvidence.frontend_commit_sha, "710f23525244083db4927186326c1a7f8ddc9ccf");
