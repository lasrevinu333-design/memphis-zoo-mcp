...
const alignTwoInternalAbsencesFile = "20260918010000_align_two_internal_absences.sql";
const alignTwoInternalAbsences = readFileSync(`supabase/migrations/${alignTwoInternalAbsencesFile}`, "utf8");
const pauseGuestNotificationsWhenDisabledFile = "20260918011000_pause_guest_notifications_when_disabled.sql";
const pauseGuestNotificationsWhenDisabled = readFileSync(`supabase/migrations/${pauseGuestNotificationsWhenDisabledFile}`, "utf8");
...
const canonicalFingerprint = readFileSync("supabase/canonical/schema-fingerprint.txt", "utf8").trim();
...
assert.match(releaseInput.cutover.phase_order[3], /three exact ordered pending migrations.*lunch-delivery manager alerts.*completed-cleaning reminder compatibility.*verified-visit five-minute overdue authority.*ledger to advance exactly three entries.*do not replay historical production migrations/i);
...
assert.match(releaseInput.cutover.production_migration_evidence.task_local,
  /host networking.*db\.rqquvtjdmugpigbndmne\.supabase\.co.*verify-full.*encrypted archive stays local.*same isolated restore.*three-migration.*runtime.*write.*replay.*cleanup/i);
...
assert.equal(productionMigrationState.observed_production.catalog_capture_format, "canonical_target.v1 expected-current; locked apply must recapture and compare exact catalog before DDL");
assert.equal(productionMigrationState.observed_production.public_function_count, 506);
...
assert.equal(productionMigrationState.observed_production.command_receipt_count, 19);
assert.equal(productionMigrationState.observed_production.authority_revision, 19);
assert.equal(productionMigrationState.observed_production.projection_count, 4);
assert.equal(productionMigrationState.observed_production.occurrence_count, 1394);
assert.equal(productionMigrationState.observed_production.application_reader_identity_policy_count, 15);
...
assert.equal(productionMigrationState.target.source_migration_file, verifiedVisitReminderFile);
...
assert.match(alignTwoInternalAbsences, /cardinality\(v_internal_ids\) <> 2/i);
assert.match(pauseGuestNotificationsWhenDisabled, /j\.job_type <> 'guest_cleanliness_report'/i);
assert.match(eventReminderWorkdayAndOwnerContract, /three_days_before.*two_days_before.*shift_plus_15/is);
assert.match(eventReminderStaticWeeklyAuthority, /projectionAvailability/);
assert.match(eventReminderStaticWeeklyAuthority, /static_weekly_v6_schedule_authority_state/);
assert.match(eventReminderStaticWeeklyAuthority, /static_weekly_v6_read_schedule_segments/);
assert.deepEqual(
...
assert.equal(new Set(exactMigrationFiles.map((file) => file.slice(0, 14))).size, exactMigrationFiles.length,
  "migration inventory contains a duplicate version");
const exactAppliedReleaseMigrations = exactMigrationFiles.filter(
  (file) => file.slice(0, 14) <= productionMigrationState.observed_production.ledger_head,
);
const exactUnappliedReleaseMigrations = exactMigrationFiles.filter(
  (file) => file.slice(0, 14) > productionMigrationState.observed_production.ledger_head,
);
assert.equal(exactAppliedReleaseMigrations.length, productionMigrationState.observed_production.source_authority_migration_count,
  "captured source migration count must equal the complete applied prefix");
assert.equal(exactAppliedReleaseMigrations.at(-1),
  `${productionMigrationState.observed_production.ledger_head}_${productionMigrationState.observed_production.source_migration_name}.sql`,
  "captured ledger head must be the final migration in the complete applied prefix");
assert.deepEqual(exactUnappliedReleaseMigrations, exactPendingReleaseMigrations,
  "the complete migration suffix after the captured ledger head must equal the exact pending corrections");
...
assert.equal(releaseEvidence.migrations.at(-1).name, verifiedVisitReminderFile);
...
assert.equal(releaseEvidence.frontend_commit_sha, "fb3cb815a2ebd8f50b33f657c750807f3b4b45b4");
...
const observedCatalog = JSON.parse(readFileSync("release/production-schema-observation-20260922.json", "utf8"));
assert.equal(observedCatalog.project_ref, productionMigrationState.project_ref);
assert.equal(observedCatalog.ledger_head, productionMigrationState.observed_production.ledger_head);
assert.equal(observedCatalog.ledger_count, productionMigrationState.observed_production.production_ledger_count);
assert.equal(observedCatalog.canonical_catalog_fingerprint, productionMigrationState.observed_production.catalog_privilege_fingerprint);
assert.equal(Object.keys(observedCatalog.catalog_arrays).length, 23);
assert.deepEqual(Object.keys(observedCatalog.catalog_arrays).sort(), Object.keys(canonicalCatalog).sort());
for (const [section, proof] of Object.entries(observedCatalog.catalog_arrays)) {
  assert.match(proof.actual, /^[0-9a-f]{64}$/);
  assert.equal(proof.actual, proof.expected, `source catalog section differs: ${section}`);
  assert.equal(proof.equal, true);
}
assert.deepEqual(observedCatalog.catalog_counts, productionMigrationState.observed_production.catalog_counts);
assert.equal(observedCatalog.unsupported_foreign_tables, 0);
assert.equal(observedCatalog.unsupported_composite_types, 0);
assert.equal(productionMigrationState.previous_observation.production_ledger_count, 226);
assert.equal(productionMigrationState.previous_observation.ledger_head, "20260916010000");
