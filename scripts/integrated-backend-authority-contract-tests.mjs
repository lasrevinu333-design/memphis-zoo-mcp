...
assert.equal(releaseEvidence.migrations.at(-1).name, "20260813210000_custodial_u4_ops_closure.sql");
...
assert.match(releaseEvidence.compatibility_window.u4_ops_closure_phase,
  /wire timestamps.*activation boundaries.*UUID completion.*two-phase employee notification dispatch ledgers.*notification recovery authority.*terminal notification retries.*authority set/);
...
assert.equal(releaseEvidence.frontend_commit_sha, "4d3278c9ec6267c2d38bccb64dc13a2e76a14096");
...
assert.equal(releaseEvidence.authority_content_identity.migration_path_count, 76);
assert.equal(releaseEvidence.migrations.length, 76);
