...
assert.match(index, /app\.post\("\/admin-api\/close-ticket"[\s\S]{0,700}runRpc\("close_maintenance_ticket"/);
assert.match(index, /app\.post\("\/dashboard-api\/close-ticket"[\s\S]{0,700}runRpc\("close_maintenance_ticket"/);
...
assert.equal(releaseEvidence.migrations.at(-1).name, "20260810170000_finish_offline_authority_operational_closure.sql");
assert.equal(releaseEvidence.cutover.source_identity, "agent/integrated-backend-10e5952-audit-correction-20260810");
