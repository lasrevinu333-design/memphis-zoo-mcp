...
try {
  ...
  for (const lang of ['en','es','both']) {
    const response=await fetch(`${base}/coverall/assignment?service_date=2026-07-30&slot=COVERALL_01&lang=${lang}&access_token=${token}`);
    assert.equal(response.status,200);
    const html=await response.text();
    assert.ok(html.includes(`data-assignment-language="${lang}"`));
    assert.ok(html.includes('@media print'));
    if(lang==='both') {
      for(const label of ['CoverAll Assignments / Asignaciones de CoverAll','Shift / Turno','Public restrooms / Baños públicos','Exhibits / Exhibiciones'])assert.ok(html.includes(label),label);
    }
    assert.ok(html.includes('slot=COVERALL_01'));
    assert.ok(!html.includes('slot=COVERALL_02'),'public output must remain scoped to the one approved contractor slot');
  }
  assert.equal(writeCount,0,'bilingual rendering remains read-only');
