...
assert.equal(pendingMigrationPlan.target?.source_migration_file, scanAlertRuntimeAuthorityClosure);
assert.equal(pendingMigrationPlan.target?.source_migration_file, pendingProductionMigrations.at(-1));
assert.equal(pendingMigrationPlan.target?.source_migration_name, "scan_alert_runtime_authority_closure");
