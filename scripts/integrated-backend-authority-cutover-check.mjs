...
const staticWeeklyEndOfDayWindowParity = "20260823113624_static_weekly_end_of_day_window_parity.sql";
...
assert.equal(expectedBlobs.filter(({ path }) => /^supabase\/migrations\/[^/]+\.sql$/.test(path)).length, 101, "release authority inventory must bind all 101 migrations");
...
assert.equal(pendingMigrationPlan.target?.source_migration_file, staticWeeklyEndOfDayWindowParity);
