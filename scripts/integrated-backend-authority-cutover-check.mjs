...
const staticWeeklyProjectionCompactSnapshot = "20260824170000_static_weekly_projection_compact_snapshot.sql";
...
assert.equal(expectedBlobs.filter(({ path }) => /^supabase\/migrations\/[^/]+\.sql$/.test(path)).length, 108, "release authority inventory must bind all 108 migrations");
...
assert.equal(pendingMigrationPlan.target?.source_migration_file, staticWeeklyProjectionCompactSnapshot);
assert.equal(pendingMigrationPlan.target?.source_migration_file, pendingProductionMigrations.at(-1));
assert.equal(pendingMigrationPlan.target?.source_migration_name, "static_weekly_projection_compact_snapshot");
