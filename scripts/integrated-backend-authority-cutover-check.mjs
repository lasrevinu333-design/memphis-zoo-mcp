...
const staticWeeklyOperationalTruthCutover = "20260824213000_static_weekly_operational_truth_cutover.sql";
...
assert.equal(expectedBlobs.filter(({ path }) => /^supabase\/migrations\/[^/]+\.sql$/.test(path)).length, 110, "release authority inventory must bind all 110 migrations");
...
assert.equal(pendingMigrationPlan.target?.source_migration_file, staticWeeklyOperationalTruthCutover);
assert.equal(pendingMigrationPlan.target?.source_migration_file, pendingProductionMigrations.at(-1));
assert.equal(pendingMigrationPlan.target?.source_migration_name, "static_weekly_operational_truth_cutover");
