...
const staticWeeklyInitialContractorSnapshot = "20260824132000_static_weekly_initial_contractor_snapshot.sql";
...
assert.equal(expectedBlobs.filter(({ path }) => /^supabase\/migrations\/[^/]+\.sql$/.test(path)).length, 107, "release authority inventory must bind all 107 migrations");
...
assert.equal(pendingMigrationPlan.target?.source_migration_file, staticWeeklyInitialContractorSnapshot);
assert.equal(pendingMigrationPlan.target?.source_migration_file, pendingProductionMigrations.at(-1));
assert.equal(pendingMigrationPlan.target?.source_migration_name, "static_weekly_initial_contractor_snapshot");
