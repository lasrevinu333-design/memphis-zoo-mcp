...
const initialDraftRosterHydration = "20260827024500_static_weekly_initial_draft_roster_hydration.sql";
const registeredSourceDatedStatus = "20260827033500_static_weekly_registered_source_dated_status.sql";
const disasterRecoveryMutationFence = "20260827150000_disaster_recovery_global_mutation_fence.sql";
const applicationReaderFeedbackRuntime = "20260827151000_restore_application_reader_feedback_runtime.sql";
const outlookEventSyncAuthority = "20260827152000_adopt_outlook_event_sync_authority.sql";
...
const lunchDeliveryFailureManagerAlert = "20260922050000_lunch_delivery_failure_manager_alert.sql";
const completedCleaningReminderCycles = "20260922070000_completed_cleaning_reminder_cycles.sql";
...
assert.equal(productionMigrationState.pending_migrations.length, 3, "the candidate must identify exactly the three ordered Sep22 corrections");
...
const initialDraftRosterHydrationText = blobByPath.get(`supabase/migrations/${initialDraftRosterHydration}`).bytes.toString("utf8");
const registeredSourceDatedStatusText = blobByPath.get(`supabase/migrations/${registeredSourceDatedStatus}`).bytes.toString("utf8");
const disasterRecoveryMutationFenceText = blobByPath.get(`supabase/migrations/${disasterRecoveryMutationFence}`).bytes.toString("utf8");
const applicationReaderFeedbackRuntimeText = blobByPath.get(`supabase/migrations/${applicationReaderFeedbackRuntime}`).bytes.toString("utf8");
const outlookEventSyncAuthorityText = blobByPath.get(`supabase/migrations/${outlookEventSyncAuthority}`).bytes.toString("utf8");
const releaseHealthIdentityProjectionText = blobByPath.get(`supabase/migrations/${releaseHealthIdentityProjection}`).bytes.toString("utf8");
const messagingDurabilityText = blobByPath.get(`supabase/migrations/${messagingDurability}`).bytes.toString("utf8");
const nativeStartOperationalTruthText = blobByPath.get(`supabase/migrations/${nativeStartOperationalTruth}`).bytes.toString("utf8");
...
assert.match(input.cutover.phase_order[3], /three exact ordered pending migrations.*lunch-delivery manager alerts.*completed-cleaning reminder compatibility.*verified-visit five-minute overdue authority.*ledger to advance exactly three entries.*do not replay historical production migrations/i);
...
assert.equal(productionMigrationState.target?.source_migration_file, verifiedVisitReminderState);
assert.equal(productionMigrationState.target?.source_migration_name, "verified_visit_reminder_state");
assert.equal(productionMigrationState.target?.source_migration_version, "20260922090000");
...
assert.equal(productionMigrationState.target?.public_function_count, 512);
assert.equal(productionMigrationState.target?.production_ledger_count, 233);
...
assert.equal(productionMigrationState.target?.expected_catalog_counts?.functions, 512);
...
assert.match(messagingDurabilityText, /msg_mark_thread_read_through/);
assert.match(messagingDurabilityText, /p_client_message_id/);
assert.match(nativeStartOperationalTruthText, /create or replace function public\.custodial_open_work\(\)/i);
assert.match(nativeStartOperationalTruthText, /create or replace function public\.expire_stale_open_sessions\(/i);
assert.match(nativeStartOperationalTruthText, /v_location_dashboard_status/i);
...
for (const [index, migration] of productionMigrationState.pending_migrations.entries()) {
  ...
  assert.match(String(migration.source_migration_version || ""), /^[0-9]{14}$/, `pending source migration version is invalid at ${migration.file}`);
...
assert.match(credentialReplacementLineageText, /custodial_device_credential_replacements/);
...
assert.match(initialDraftRosterHydrationText, /static_weekly_v3_read_authority_source\(\s*p_source_id uuid,\s*p_service_date date\s*\)/i);
assert.match(initialDraftRosterHydrationText, /static_weekly_v4_hydrate_compiler_source/i);
assert.match(initialDraftRosterHydrationText, /grant\s+execute\s+on\s+function\s+public\.static_weekly_v3_read_authority_source\(uuid,date\)\s+to\s+static_weekly_control_plane/i);
assert.doesNotMatch(initialDraftRosterHydrationText, /grant\s+execute\s+on\s+function\s+public\.static_weekly_v3_read_authority_source\(uuid,date\)\s+to\s+(?:public|anon|authenticated|service_role|static_weekly_release_operator|custodial_application_reader)/i);
assert.match(registeredSourceDatedStatusText, /static_weekly_v5_registered_source_identity\(p_source jsonb\)/i);
assert.match(registeredSourceDatedStatusText, /item-'status'/i);
assert.doesNotMatch(registeredSourceDatedStatusText, /-'vacancyCapableSlotIds'/i);
assert.match(registeredSourceDatedStatusText, /custodial_release_authority_restore_inventory/i);
assert.doesNotMatch(registeredSourceDatedStatusText, /grant\s+execute\s+on\s+function\s+public\.static_weekly_v5_registered_source_identity\(jsonb\)\s+to\s+(?:public|anon|authenticated|service_role|static_weekly_release_operator|custodial_application_reader)/i);
...
assert.match(applicationReaderFeedbackRuntimeText, /custodial_release_authority_restore_inventory/i);
assert.match(outlookEventSyncAuthorityText, /create table if not exists public\.events_app_outlook_sync/i);
assert.match(outlookEventSyncAuthorityText, /enable row level security/i);
assert.match(outlookEventSyncAuthorityText, /grant delete, insert, maintain, references, select, trigger, truncate, update[\s\S]*to service_role/i);
assert.match(outlookEventSyncAuthorityText, /revoke all privileges on table public\.events_app_outlook_sync[\s\S]*from public, anon, authenticated, service_role, custodial_application_reader,[\s\S]*postgres, supabase_admin/i);
assert.match(outlookEventSyncAuthorityText, /No mailbox credentials are stored/i);
assert.doesNotMatch(outlookEventSyncAuthorityText, /grant\s+[^;]+\s+to\s+(?:anon|authenticated|custodial_application_reader)\b/i);
...
const result = {
  ...
  applied_release_migrations: appliedReleaseMigrations,
  pending_migrations: pendingReleaseMigrations,
  production_state: "migration_required",
