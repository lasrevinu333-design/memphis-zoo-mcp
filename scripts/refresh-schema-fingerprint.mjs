...
if(!/^(postgres|mz_schema_rebuild_[a-zA-Z0-9_]+)$/.test(database)||!/^mz_schema_rebuild_[a-zA-Z0-9_]+$/.test(container))throw new Error("A disposable mz_schema_rebuild Docker container and database are required.");
...
const inventory=Object.fromEntries(Object.entries(queries).map(([name,sql])=>[name,query(sql)]));
