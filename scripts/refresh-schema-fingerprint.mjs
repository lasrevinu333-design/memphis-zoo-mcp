...
const queries={
  ...
  views:`select n.nspname as schema_name,c.relname as view_name,c.relkind as relation_kind,pg_get_viewdef(c.oid,true) as definition,obj_description(c.oid,'pg_class') as object_comment from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relkind in ('v','m') order by c.relname`,
