...
export const SCHEMA_CATALOG_QUERIES = Object.freeze({
  ...
  cron_jobs: `select jobname,schedule,command,database,case when username in ('postgres','supabase_admin') then 'migration_owner' else username end as username,active from cron.custodial_schema_identity_cron_jobs() order by jobname`,
});
