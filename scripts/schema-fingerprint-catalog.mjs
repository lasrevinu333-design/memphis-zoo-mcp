...
export const SCHEMA_CATALOG_QUERIES = Object.freeze({
  ...
  default_privileges: `select 'migration_owner' as owner_role,n.nspname as schema_name,d.defaclobjtype as object_type,coalesce(grantee.rolname,'PUBLIC') as grantee,'migration_owner' as grantor,x.privilege_type,x.is_grantable from pg_default_acl d join pg_roles owner on owner.oid=d.defaclrole left join pg_namespace n on n.oid=d.defaclnamespace cross join lateral aclexplode(d.defaclacl) x left join pg_roles grantee on grantee.oid=x.grantee where owner.rolname=current_user and (n.nspname='public' or d.defaclnamespace=0) and coalesce(grantee.rolname,'PUBLIC') not in ('postgres','supabase_admin') order by n.nspname,d.defaclobjtype,grantee,x.privilege_type`,
