...
export const SCHEMA_CATALOG_QUERIES = Object.freeze({
  ...
  default_privileges: `select owner.rolname as owner_role,n.nspname as schema_name,d.defaclobjtype as object_type,coalesce(grantee.rolname,'PUBLIC') as grantee,grantor.rolname as grantor,x.privilege_type,x.is_grantable from pg_default_acl d join pg_roles owner on owner.oid=d.defaclrole left join pg_namespace n on n.oid=d.defaclnamespace cross join lateral aclexplode(d.defaclacl) x left join pg_roles grantee on grantee.oid=x.grantee left join pg_roles grantor on grantor.oid=x.grantor where n.nspname='public' or d.defaclnamespace=0 order by owner.rolname,n.nspname,d.defaclobjtype,grantee,grantor,x.privilege_type`,
