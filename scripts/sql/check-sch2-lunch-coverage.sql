select *
from (
  -- SCH2 lunch coverage readiness/audit gate.
  -- Returns rows until same_lunch and overlap checks are present in constraint audit substrate.
  with required_relations(object_name, object_kind) as (
    ...
    select
      'missing_lunch_audit_relation'::text as violation_type,
      rr.object_name,
      rr.object_kind,
      'SCH2 lunch coverage audit relation is not present yet'::text as detail
    from required_relations rr
    where to_regclass(rr.object_name) is null
  ), missing_columns as (
    select
      'missing_lunch_audit_column'::text as violation_type,
      ec.object_name || '.' || ec.column_name as object_name,
      'column'::text as object_kind,
      'Required lunch audit column is not present yet'::text as detail
    ...
    select
      'missing_lunch_violation_label'::text as violation_type,
      label as object_name,
      'audit_label'::text as object_kind,
      'Constraint view must expose lunch coverage ' || label || ' failures'::text as detail
    from (values ('same_lunch'), ('overlap'), ('lunch_coverage')) labels(label)
