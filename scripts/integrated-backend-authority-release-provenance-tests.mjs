...
try {
  ...
  assert.equal(
    payload.source_identity.migration_path_count,
    positive.trackedPaths.filter((path) => /^supabase\/migrations\/[^/]+\.sql$/.test(path)).length,
    "signed release provenance must derive the migration count from the exact tracked fixture tree",
  );
