...
const defaultScopeConfig = assertMcpOAuthConfig({ ...BASE_ENV, MCP_OAUTH_SCOPES: "" });
assert.deepEqual(defaultScopeConfig.scopes, ["email"]);
const challenge = buildMcpWwwAuthenticateChallenge(config);
assert.match(challenge, /^Bearer resource_metadata="https:\/\/memphis-zoo-mcp\.onrender\.com\/\.well-known\/oauth-protected-resource\/mcp"/);
...
assert.throws(
  () => assertMcpOAuthConfig({ ...BASE_ENV, MCP_PUBLIC_URL: "http://127.0.0.1:3000" }),
  /must use HTTPS/,
  "Production configuration must not permit local HTTP origins.",
);
assert.equal(
  assertMcpOAuthConfig({
    ...BASE_ENV,
...
assert.throws(
  () => assertMcpOAuthConfig({ ...BASE_ENV, SUPABASE_PUBLISHABLE_KEY: "sb_secret_wrong-key-class" }),
  /must not contain the Supabase secret or service-role key/,
);
assert.throws(
  () => assertMcpOAuthConfig({ ...BASE_ENV, MCP_OAUTH_CLOCK_SKEW_SECONDS: "301" }),
  /MCP_OAUTH_CLOCK_SKEW_SECONDS must be an integer between 0 and 300/,
);
...
assert.equal((await verifier.verifyAccessToken(resourceClaimToken)).extra.resource, config.resource);
...
await assert.rejects(
  ...
  McpOAuthTokenError,
  "JWT NumericDate claims must be JSON numbers, not coercible strings.",
);
assert.equal(getUserCalls, 2, "Unallowlisted claims must be rejected before a network lookup.");
await assert.rejects(
  verifier.verifyAccessToken(jwt({
    iss: config.issuer,
    sub: SUBJECT,
...
await assert.rejects(
  ...
  McpOAuthTokenError,
  "A valid identity token minted for another resource must be rejected.",
);
await assert.rejects(
  verifier.verifyAccessToken(jwt({
    iss: config.issuer,
  ...
  McpOAuthTokenError,
  "Every configured MCP scope must be present in the access token.",
);
await assert.rejects(
  verifier.verifyAccessToken(jwt({
    iss: config.issuer,
  ...
  McpOAuthTokenError,
  "A token that is not yet valid beyond the configured clock skew must be rejected.",
);
assert.equal(getUserCalls, 2, "Invalid audience, scope, and time claims must fail before network validation.");
...
const protectedServer = createMcpServer({
  name: "mcp-oauth-tool-error-contract",
  version: "test",
  oauth: {
    enabled: true,
...
const protectedClient = new Client({ name: "mcp-oauth-tool-error-client", version: "test" });
const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
try {
  await protectedServer.connect(serverTransport);
  await protectedClient.connect(clientTransport);
  const authError = await protectedClient.callTool({ name: "ping", arguments: {} });
  assert.equal(authError.isError, true);
  assert.deepEqual(authError._meta?.["mcp/www_authenticate"], [challenge]);
  assert.match(String(authError.content?.[0]?.text || ""), /Authentication required/);
} finally {
  await protectedClient.close().catch(() => {});
  await protectedServer.close().catch(() => {});
}
