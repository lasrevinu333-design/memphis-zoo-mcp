export function buildIntegratedBackendAuthorityReleaseEvidence({
  ...
  return {
    ...
    compatibility_window: {
      ...
      credential_replacement_lineage_phase: "20260826020000 preserves frozen offline work across manager-approved credential recovery through append-only same-device predecessor-to-successor transport lineage without rewriting the original actor, device, credential, or work evidence",
      application_reader_runtime_recovery_phase: "20260826155000 restores restricted Messenger reads and 20260826160000 restores restricted Employee Events reads without granting browser roles direct relation authority",
      static_weekly_vacancy_and_lunch_phase: "20260827010500 adds first-class empty schedule positions, requires every vacant shift to retain a valid lunch inside its shift, keeps vacant work OPEN without synthetic identity, and permits only the static-weekly control plane to create or fill a position",
      disaster_recovery_foundation_phase: "20260827150000 fences application mutations across an exact quiescent restore lease and restores scheduler state before traffic resumes; 20260827151000 restores the bounded feedback reader projection, RLS policy, and recovery inventory without exposing legacy images; 20260827152000 adopts the production Outlook event-sync audit ledger without rewriting its rows or granting browser authority; 20260827153000 scopes release-health identity checks to the exact employee/device projection while retaining separately inventoried feedback authority",
      native_start_operational_truth_phase: "20260916010000 projects an attested native NFC Start into manager, scan, GPS, rollback, health, and admission truth without fabricating a sessions row; expires stale projected work into terminal cancelled evidence; and inventories every changed function, view, grant, and index for exact recovery",
