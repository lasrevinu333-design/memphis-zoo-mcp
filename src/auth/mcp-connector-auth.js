...
export function authenticateMcpConnectorRequest(
  ...
  // Tokenless access is restricted to the separately enabled manifest READ
  // surface. It never reaches GitHub or Supabase mutators. A presented but
...
export function makeMcpConnectorMiddleware(
  ...
  return async function requireMcpConnectorAuth(req, res, next) {
    ...
    if (!result.ok) {
      const challenge = oauthChallenge
        || (resourceMetadataUrl
          ...
          : null);
      if ((result.status || 401) === 401 && challenge) {
      ...
      if (challenge) body._meta = { "mcp/www_authenticate": [challenge] };
    ...
    if (result.auth_info) {
      req.auth = result.auth_info;
    } else if (result.auth_source === "connector_token") {
      // The MCP SDK uses req.auth only as verified request context. Do not copy
      // the long-lived legacy service token into handler-visible metadata.
      req.auth = {
        token: "verified-legacy-connector-token",
        clientId: "memphis-mcp-connector-token",
        scopes: [],
        extra: { authSource: "connector_token" },
      };
    }
    next();
  };
