...
const SUPPORTED_SCOPES = new Set(["openid", "email", "profile", "phone", "offline_access"]);
// `email` is supported by Supabase OAuth without requiring the asymmetric
// signing-key prerequisite that applies when `openid` ID tokens are requested.
const DEFAULT_SCOPES = ["email"];
...
function configuredBoundedInteger(name, value, fallback, maximum, errors, { minimum = 0 } = {}) {
  if (value == null || String(value).trim() === "") return fallback;
...
export function getMcpOAuthConfig(env = process.env) {
  ...
  const errors = [];
  // Local HTTP exists only for deterministic process-level tests. A production
  // typo must not silently turn the OAuth issuer or protected resource into a
  // bearer-token endpoint without transport security.
  const allowLocalHttp = String(env.NODE_ENV || "").trim().toLowerCase() === "test";
  const publicOrigin = parseOrigin("MCP_PUBLIC_URL", env.MCP_PUBLIC_URL, errors, { allowLocalHttp });
  const supabaseOrigin = parseOrigin("SUPABASE_URL", env.SUPABASE_URL, errors, { allowLocalHttp });
  ...
  const uiSessionSeconds = configuredBoundedInteger(
    "MCP_OAUTH_UI_SESSION_SECONDS",
  ...
  const csrfSeconds = configuredBoundedInteger(
    "MCP_OAUTH_CSRF_SECONDS",
    env.MCP_OAUTH_CSRF_SECONDS,
    DEFAULT_CSRF_SECONDS,
    30 * 60,
    errors,
    { minimum: 60 },
  );
  const clockSkewSeconds = configuredBoundedInteger(
    "MCP_OAUTH_CLOCK_SKEW_SECONDS",
    env.MCP_OAUTH_CLOCK_SKEW_SECONDS,
  ...
  if (/^sb_secret_/i.test(publishableKey) || (serviceRoleKey && publishableKey === serviceRoleKey)) {
    errors.push("SUPABASE_PUBLISHABLE_KEY must not contain the Supabase secret or service-role key.");
  }
...
function quotedAuthParameter(value) {
  return String(value).replaceAll("\\", "\\\\").replaceAll('"', '\\"');
}

export function buildMcpWwwAuthenticateChallenge(
  config,
  { error = "invalid_token", errorDescription = "Authentication is required or the access token is invalid." } = {},
) {
  if (!config?.enabled || !config?.ready) throw new Error("MCP OAuth must be enabled and configured.");
  const values = [
    `resource_metadata="${quotedAuthParameter(config.resourceMetadataUrl)}"`,
    `scope="${quotedAuthParameter(config.scopes.join(" "))}"`,
  ];
  if (error) values.push(`error="${quotedAuthParameter(error)}"`);
  if (errorDescription) values.push(`error_description="${quotedAuthParameter(errorDescription)}"`);
...
function stringClaimValues(value) {
  if (Array.isArray(value)) return value.map(String).filter(Boolean);
  if (typeof value === "string" && value.trim()) return value.trim().split(/\s+/).filter(Boolean);
  return [];
}

function tokenIsResourceBound(payload, expectedResource) {
  const audiences = stringClaimValues(payload.aud);
  const resources = stringClaimValues(payload.resource);
  return audiences.includes(expectedResource) || resources.includes(expectedResource);
}
...
export function createMcpOAuthVerifier({ env = process.env, supabaseClient, now = () => Date.now() } = {}) {
  ...
  return {
    ...
    async verifyAccessToken(token) {
      ...
      const notBefore = payload.nbf == null
        ? null
        : (typeof payload.nbf === "number" ? payload.nbf : Number.NaN);
      ...
      if (
        ...
        || expiresAt <= nowSeconds - config.clockSkewSeconds
        || (notBefore != null && (!Number.isFinite(notBefore) || notBefore > nowSeconds + config.clockSkewSeconds))
        || !tokenIsResourceBound(payload, config.resource)
