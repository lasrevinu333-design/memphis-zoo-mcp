...
function publicCore(problem, solved) {
  ...
  const core = {
    ...
    certificate: { schema: "memphis-zoo.static-weekly-solver-certificate.v4", compilerVersion: STATIC_WEEKLY_SCHEDULER_VERSION, verifierVersion: "static-weekly-js-verifier-v4-monotonic-leximax", objectivePolicyVersion: "monotonic-leximax-v1", canonicalInputDigest: problem.inputDigest, baselineInputDigest: problem.baselineInputDigest, weeklyVersionDigest: problem.weeklyVersionDigest, solverIdentity: solved.final.identity, options: solved.tiers.map((tier) => tier.options), assignmentDigest: sha256Hex(canonicalJson(weeklyAssignments.map((assignment) => ({ planWorkId: assignment.planWorkId, status: assignment.status, slotId: assignment.slotId, serviceDate: assignment.serviceDate })))), canonicalProgram: solved.canonicalProgram, modelBasis: solved.modelBasis, modelBasisDigest: sha256Hex(canonicalJson(solved.modelBasis)), finalWitness: solved.finalWitness, execution: solved.execution, tiers: solved.tiers },
    assignments, weeklyAssignments, openWork, reviewWork,
