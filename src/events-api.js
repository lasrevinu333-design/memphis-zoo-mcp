...
async function deleteEventRecord(runWriteSql, eventId) {
  ...
  await runWriteSql("events_app_delete", `delete from public.events_app_events where id = ${sqlLiteral(normalizedId)}::uuid;`);
  ...
  await runWriteSql(
    ...
    `insert into public.events_app_notification_log (
       ...
       ${kind === "day_before"
         ...
         : `(${sqlLiteral(eventRow.event_date)}::date + ${sqlLiteral(assignmentRow.coverage_start || "00:00:00")}::time + interval '15 minutes')`},
       now(),
       'sent',
