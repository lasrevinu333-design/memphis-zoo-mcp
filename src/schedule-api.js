...
export function createScheduleRouter({
  ...
  function coverAllEmployeeCode(index = 1) {
    const n = Math.max(1, Math.min(4, Number.parseInt(String(index || 1), 10) || 1));
    return `COVERALL${String(n).padStart(2, "0")}`;
  }

  async function ensureCoverAllEmployees(count = 1) {
    const safeCount = Math.max(1, Math.min(4, Number.parseInt(String(count || 1), 10) || 1));
    if (typeof runWriteSql !== "function") throw new Error("CoverAll write path is not configured.");
    const values = Array.from({ length: safeCount }, (_v, index) => {
      const code = coverAllEmployeeCode(index + 1);
      const name = `CoverAll ${String(index + 1).padStart(2, "0")}`;
      return `('${esc(code)}', '${esc(name)}', true, 'contractor', 'Third-party CoverAll custodial contractor slot ${index + 1}.')`;
    }).join(",\n");
    await runWriteSql("coverall_slots_seed", `
      ...
      update public.employees
         set active = true, role = 'contractor', updated_at = now()
       where employee_code in (${Array.from({ length: safeCount }, (_v, index) => `'${esc(coverAllEmployeeCode(index + 1))}'`).join(",")});
    `);
    const rows = await runReadOnlySql(`
      select id as employee_id, display_name, employee_code
      from public.employees
    ...
    `);
    if (!Array.isArray(rows) || rows.length < safeCount) throw new Error("Could not create or find requested CoverAll slots.");
    return rows.slice(0, safeCount);
  }

  async function getActiveCoverAllCount(serviceDate) {
    const rows = await runReadOnlySql(`
      ...
      where dwr.service_date = '${esc(serviceDate)}'::date
        and dwr.active = true
        and e.employee_code ~* '^COVERALL[0-9]{2}(serviceDate, explicitIds = []) {
    const explicit = normalizeUuidList(explicitIds);
...
/* malformed duplicate tail disabled
  ...
  async function setCoverAllCount(serviceDate, count = 0) {
    const safeCount = Math.max(0, Math.min(4, Number.parseInt(String(count || 0), 10) || 0));
    const slots = safeCount ? await ensureCoverAllEmployees(safeCount) : await ensureCoverAllEmployees(1);
    const activeIds = slots.slice(0, safeCount).map((row) => `'${esc(row.employee_id)}'::uuid`).join(",") || "null::uuid";
    const allCodes = Array.from({ length: 4 }, (_v, index) => `'${esc(coverAllEmployeeCode(index + 1))}'`).join(",");
    await runWriteSql("coverall_count_set", `
      update public.daily_work_roster dwr
         set active = false, updated_at = now(), notes = 'CoverAll slot disabled for this date.'
        from public.employees e
       where dwr.employee_id = e.id
         and dwr.service_date = '${esc(serviceDate)}'::date
         and e.employee_code in (${allCodes});

      insert into public.daily_work_roster (service_date, employee_id, shift_start, shift_end, source_type, notes, active, created_at, updated_at)
      select '${esc(serviceDate)}'::date, x.employee_id, '07:00'::time, '18:00'::time, 'coverall',
             'CoverAll contractor present. Assignments may be split by absence escalation.', true, now(), now()
      from (select unnest(array[${activeIds}]) as employee_id) x
      where x.employee_id is not null
        and not exists (
          select 1 from public.daily_work_roster existing
          where existing.service_date = '${esc(serviceDate)}'::date
            and existing.employee_id = x.employee_id
        );

      update public.daily_work_roster dwr
         set active = true, shift_start = least(dwr.shift_start, '07:00'::time), shift_end = greatest(dwr.shift_end, '18:00'::time),
             source_type = 'coverall', notes = 'CoverAll contractor present. Assignments may be split by absence escalation.', updated_at = now()
       where dwr.service_date = '${esc(serviceDate)}'::date
         and dwr.employee_id in (${activeIds});
    `);
    return { service_date: serviceDate, coverall_count: safeCount, slots: slots.slice(0, safeCount) };
  }
