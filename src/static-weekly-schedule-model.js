...
// A small synchronous SHA-256 implementation keeps browser/database replay
// pure. The isolated production compiler may install a validated native
...
function portableSha256Hex(text) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const next1 = (h + sigma1 + choose + SHA256_CONSTANTS[index] + words[index]) >>> 0;
...
export function installStaticWeeklySha256HexAccelerator(implementation) {
  ...
  const vectors = [
    ...
    ["Memphis Zoo Custodial \ud83d\udcf1", null],
  ];
  for (const [input, known] of vectors) {
    const portable = portableSha256Hex(input);
...
// This remains the single digest entry point for compiler, verifier, browser,
// worker, and database fixtures, so native acceleration cannot alter identity.
export function sha256Hex(text) {
  const input = String(text);
  return SHA256_HEX_ACCELERATOR ? SHA256_HEX_ACCELERATOR(input) : portableSha256Hex(input);
}
