-- Ensure every active employee Messenger identity has exactly one usable Memphis conversation.
-- The API also calls msg_get_or_create_memphis_thread during thread listing, so future users self-heal.
