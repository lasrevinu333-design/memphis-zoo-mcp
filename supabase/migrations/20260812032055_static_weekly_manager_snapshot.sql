-- Read-only manager snapshot for the static weekly scheduler.
--
-- The manager UI needs one coherent read from the same authority that accepts
-- scheduler commands. Direct table reads remain unavailable to the runtime.
begin;
...
begin
  ...
  if v_publication_id is not null then
    ...
    -- The manager needs the accepted payload identity (not only its digest) to
    -- render active absences, reversals, and contractor capacity truthfully.
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'contractor_capacity',coalesce((
      ...
      cross join lateral jsonb_array_elements(v.draft_document#>'{semantic_snapshot,recurring_source,slots}') slot
...
'One read-only, Monday-aligned manager snapshot of current scheduler revision, registered source choices, effective publication, exact-week drafts, roster, availability, assignments, accepted exceptions, and latest compiled projection. Direct table access remains unavailable to the scheduler runtime.';
