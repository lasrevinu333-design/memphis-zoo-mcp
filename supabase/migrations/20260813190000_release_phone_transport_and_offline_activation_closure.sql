...
alter table public.custodial_offline_actor_contexts
  add column native_scan_entry_id uuid,
  ...
  add constraint custodial_offline_native_start_evidence_check check (
    (native_scan_entry_id is null and native_start_attestation_version is null and native_start_attestation_sha256 is null)
    or (native_scan_entry_id is not null and native_start_attestation_version='custodial-native-start.v1'
      and native_start_attestation_sha256 ~ '^[0-9a-f]{64}$')
  ),
  ...
  add constraint uq_custodial_offline_native_scan_entry unique(native_scan_entry_id);
...
declare
  ...
  v_proof_value text:=encode(extensions.gen_random_bytes(32),'hex'); v_fingerprint text; v_snapshot_location_code text;
begin
  perform public.custodial_require_backend_execution_secret(p_backend_execution_secret);
  ...
  select x->>'location_code' into v_snapshot_location_code
    from jsonb_array_elements(v_snapshot.locations_json) x
   where upper(regexp_replace(x->>'location_code','[^A-Z0-9]+','','g'))=case upper(regexp_replace(btrim(coalesce(p_location_code,'')),'[^A-Z0-9]+','','g'))
     when 'TETON' then 'TETX' when 'TETONRR' then 'TETM' when 'TETONRESTROOMS' then 'TETM'
     else upper(regexp_replace(btrim(coalesce(p_location_code,'')),'[^A-Z0-9]+','','g')) end
   order by x->>'location_code' limit 1;
  ...
  if v_existing.context_id is not null then
    if upper(btrim(coalesce(p_device_id,'')))<>upper((select device_id from public.devices where id=v_existing.device_id))
       or v_existing.credential_id<>v_credential_id or v_existing.canonical_location_code<>v_snapshot_location_code
  ...
  select d.id,d.device_id,c.confirmed_at,c.revoked_at,c.expires_at into v_device
   ...
   where upper(btrim(d.device_id))=upper(btrim(coalesce(p_device_id,''))) for update of d;
  ...
  select l.id,l.location_code,jsonb_build_array(l.location_code,upper(btrim(p_location_code))) aliases into v_location
    from public.locations l where l.location_code=v_snapshot_location_code;
  if v_location.id is null or v_snapshot_location_code is null then raise exception using errcode='22023',message='location is not authorized by the issued offline snapshot'; end if;
...
create or replace function public.custodial_start_offline_occurrence_native(
  ...
  p_native_scan_entry_id text,p_native_start_attestation_version text,p_native_start_attestation text,
  p_native_route_proof_secret text,p_backend_execution_secret text
) returns jsonb language plpgsql security definer set search_path to 'pg_catalog','public','extensions'
as $function$
declare v_result jsonb; v_context_id uuid; v_attestation_sha256 text; v_scan_entry_id uuid;
begin
  perform public.custodial_require_native_route_proof_secret(p_native_route_proof_secret);
  begin
    v_scan_entry_id:=nullif(lower(btrim(coalesce(p_native_scan_entry_id,''))),'')::uuid;
  exception when others then
    raise exception using errcode='42501',message='a verified native NFC start handoff is required';
  end;
  if v_scan_entry_id is null or btrim(coalesce(p_native_start_attestation_version,''))<>'custodial-native-start.v1'
  ...
  if exists(select 1 from public.custodial_offline_actor_contexts c where c.context_id=v_context_id
    and (c.native_start_attestation_version is not null and (c.native_scan_entry_id<>v_scan_entry_id
      or c.native_start_attestation_version<>p_native_start_attestation_version
  ...
  update public.custodial_offline_actor_contexts
     set native_scan_entry_id=v_scan_entry_id,
         native_start_attestation_version=p_native_start_attestation_version,
...
as $function$
  select public.custodial_start_offline_occurrence_native(
    p_device_id,p_location_code,p_client_session_id,p_client_started_at,p_snapshot_id,
    p_snapshot_employee_id,p_snapshot_assignment_epoch,p_snapshot_credential_id,
    p_authenticated_credential_id,p_native_scan_entry_id,p_native_start_attestation_version,
...
declare v_definition text; v_rewritten text; v_location_rewritten text;
begin
...
$new$);
  ...
  v_location_rewritten:=replace(v_rewritten,
$old$  select l.id,l.location_code,l.location_name,l.location_type,l.form_type into v_location from public.locations l where l.id=v_context.location_id and l.active=true;
$old$,
$new$  select l.id,l.location_code,l.location_name,l.location_type,l.form_type into v_location from public.locations l where l.id=v_context.location_id;
$new$);
  if v_location_rewritten=v_rewritten then raise exception 'Canonical offline completion location predicate did not match'; end if;
  execute v_location_rewritten;
end
...
with wanted(restore_order,function_identity) as (values
  (1,'public.custodial_require_backend_execution_secret(text)'),
  (2,'public.custodial_commit_offline_occurrence(text,text,text,text,text,text,jsonb,jsonb,text,text,text,text,text)'),
  (3,'public.tool_get_offline_scan_authority_snapshot(text,text,text)'),
  (4,'public.custodial_start_offline_occurrence(text,text,text,text,text,text,integer,text,text,text)'),
  (5,'public.custodial_require_native_route_proof_secret(text)'),
  (6,'public.custodial_start_offline_occurrence_native(text,text,text,text,text,text,integer,text,text,text,text,text,text,text)'),
  (7,'public.tool_start_offline_occurrence(text,text,text,text,text,text,integer,text,text,text,text,text,text,text)'),
  (8,'public.tool_commit_cleaning_workflow_authoritative(text,text,text,text,text,text,jsonb,jsonb,text,text,text,text,text,text,text,text)'),
  (9,'public.tool_complete_session_authoritative(text,jsonb,text,text,text,text)'),
  (10,'public.custodial_backend_authority_health(text)'),
  (11,'public.custodial_record_release_canary_transport_probe(text,uuid,text,text,text,text,text,uuid,text,text,text)'),
  (12,'public.custodial_get_release_canary_transport_probe_health(text,text,text,text)'),
  (13,'public.custodial_run_release_canary_recovery_probe(text,text)'),
  (14,'public.custodial_release_canary_is_paused(text,text)'),
  (15,'public.custodial_control_release_canary(uuid,uuid,text,text,text,jsonb,text)')
), captured as (
...
begin
  if (select count(*) from public.custodial_release_authority_restore_definitions)<>15 then
...
begin
  ...
  v_checks:=jsonb_build_object(
    'restore_set_complete',v_expected=15,
    ...
    'native_occurrence_evidence_present',exists(
      ...
      select 1 from information_schema.columns where table_schema='public' and table_name='custodial_offline_actor_contexts'
        and column_name='native_scan_entry_id'
    ) and exists(
      select 1 from information_schema.columns where table_schema='public' and table_name='custodial_offline_actor_contexts'
  ...
  return jsonb_build_object(
    ...
    'canonical_functions_expected',15,'canonical_functions_verified',v_expected-coalesce(cardinality(v_missing),0)-coalesce(cardinality(v_mismatched),0),
...
begin
  select execution_secret_digest into v_expected_secret_digest
    from public.custodial_backend_execution_config where config_key=true and enabled=true;
  if v_expected_secret_digest is null or length(coalesce(p_backend_execution_secret,''))<32
     or encode(extensions.digest(convert_to(p_backend_execution_secret,'UTF8'),'sha256'),'hex')<>v_expected_secret_digest then
    raise exception using errcode='42501',message='backend execution secret is not authorized';
  end if;
  ...
  if p_action='pause_canary' then
    ...
    execute 'revoke all on function public.custodial_require_backend_execution_secret(text),public.custodial_commit_offline_occurrence(text,text,text,text,text,text,jsonb,jsonb,text,text,text,text,text),public.custodial_start_offline_occurrence(text,text,text,text,text,text,integer,text,text,text),public.custodial_require_native_route_proof_secret(text),public.custodial_start_offline_occurrence_native(text,text,text,text,text,text,integer,text,text,text,text,text,text,text) from public,anon,authenticated,service_role';
    execute 'revoke all on function public.tool_start_offline_occurrence(text,text,text,text,text,text,integer,text,text,text,text,text,text,text),public.tool_get_offline_scan_authority_snapshot(text,text,text),public.tool_commit_cleaning_workflow_authoritative(text,text,text,text,text,text,jsonb,jsonb,text,text,text,text,text,text,text,text),public.tool_complete_session_authoritative(text,jsonb,text,text,text,text),public.custodial_backend_authority_health(text),public.custodial_record_release_canary_transport_probe(text,uuid,text,text,text,text,text,uuid,text,text,text),public.custodial_get_release_canary_transport_probe_health(text,text,text,text),public.custodial_run_release_canary_recovery_probe(text,text),public.custodial_release_canary_is_paused(text,text),public.custodial_control_release_canary(uuid,uuid,text,text,text,jsonb,text) from public,anon,authenticated,service_role';
    execute 'grant execute on function public.tool_start_offline_occurrence(text,text,text,text,text,text,integer,text,text,text,text,text,text,text),public.tool_get_offline_scan_authority_snapshot(text,text,text),public.tool_commit_cleaning_workflow_authoritative(text,text,text,text,text,text,jsonb,jsonb,text,text,text,text,text,text,text,text),public.tool_complete_session_authoritative(text,jsonb,text,text,text,text),public.custodial_backend_authority_health(text),public.custodial_record_release_canary_transport_probe(text,uuid,text,text,text,text,text,uuid,text,text,text),public.custodial_get_release_canary_transport_probe_health(text,text,text,text),public.custodial_run_release_canary_recovery_probe(text,text),public.custodial_release_canary_is_paused(text,text),public.custodial_control_release_canary(uuid,uuid,text,text,text,jsonb,text) to postgres,service_role';
...
set function_definition=pg_get_functiondef('public.custodial_control_release_canary(uuid,uuid,text,text,text,jsonb,text)'::regprocedure),
    definition_sha256=encode(extensions.digest(convert_to(pg_get_functiondef('public.custodial_control_release_canary(uuid,uuid,text,text,text,jsonb,text)'::regprocedure),'UTF8'),'sha256'),'hex'),
    captured_at=statement_timestamp()
where function_identity='public.custodial_control_release_canary(uuid,uuid,text,text,text,jsonb,text)';
...
revoke all on function public.custodial_configure_native_route_proof_key(text,text),
  ...
  public.custodial_start_offline_occurrence_native(text,text,text,text,text,text,integer,text,text,text,text,text,text,text),
  public.custodial_commit_offline_occurrence(text,text,text,text,text,text,jsonb,jsonb,text,text,text,text,text)
  from public,anon,authenticated,service_role;
revoke all on function public.tool_start_offline_occurrence(text,text,text,text,text,text,integer,text,text,text,text,text,text,text),
  ...
  from public,anon,authenticated,service_role;
grant execute on function public.tool_start_offline_occurrence(text,text,text,text,text,text,integer,text,text,text,text,text,text,text),
  public.tool_commit_cleaning_workflow_authoritative(text,text,text,text,text,text,jsonb,jsonb,text,text,text,text,text,text,text,text),
  public.custodial_record_release_canary_transport_probe(text,uuid,text,text,text,text,text,uuid,text,text,text),
  public.custodial_get_release_canary_transport_probe_health(text,text,text,text)
