-- My Schedule's backend reads x.source_type from sch_get_daily_schedule_with_purpose().
-- The prior function contract omitted that column even though the underlying
-- daily assignment rows carry it. This recreates the function with a named
-- source_type result while preserving the existing columns and privileges.
...
revoke all on function public.sch_get_daily_schedule_with_purpose(date)
  from public, anon, authenticated;
grant execute on function public.sch_get_daily_schedule_with_purpose(date)
  to service_role;
