...
-- Operations-first recovery: employee kiosks remain identifier-compatible until
-- a later, deliberate security rollout. No phone may be blocked by an unfinished
-- credential enrollment program.
...
-- A sync report proves that the application is alive. Keep the canonical device
-- heartbeat clock in step with the existing device_sync_status record so the
-- dashboard does not report working phones as stale.
...
-- Reconcile existing presence clocks before rebuilding the exception view.
