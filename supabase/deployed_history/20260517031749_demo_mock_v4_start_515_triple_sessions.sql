-- Deployed migration history snapshot: 20260517031749 demo_mock_v4_start_515_triple_sessions
...
begin
  ...
  v_slot := floor((v_minutes - 315) / 20.0)::integer;
  ...
  for r in
      ...
      select
        ...
        row_number() over (
          order by
            ...
            case e.coverage_purpose when 'area_owner' then 0 when 'deep_clean' then 1 when 'restroom_upkeep' then 2 when 'late_coverage' then 3 else 4 end,
            e.sort_order,
            e.location_code
        ) as pick_rank
      from eligible e
    ...
    select * from ranked
    where pick_rank <= v_wave_size
    order by pick_rank
  loop
    v_session_key := 'demo-scan:' || v_run_id::text || ':assigned-area:' || to_char(v_local_date, 'YYYYMMDD') || ':slot:' || v_slot::text || ':wave:' || r.pick_rank::text;
    ...
    insert into public.scan_events (
      ...
      jsonb_build_object(
        ...
        'coverage_purpose', r.coverage_purpose,
        'location_type', r.form_type,
        'duration_minutes', v_duration,
        'restroom_cadence_minutes', 135,
        'exhibit_cadence_minutes', 255,
        'stagger_minutes', 20,
        'work_start_central', '05:15',
    ...
    v_started_details := v_started_details || jsonb_build_array(jsonb_build_object(
      'wave', r.pick_rank,
      'location_code', r.location_code,
      'location_type', r.form_type,
      'employee_name', r.employee_name,
  ...
  set
    ...
    metadata_json = coalesce(metadata_json, '{}'::jsonb) || jsonb_build_object(
      ...
      'stagger_minutes', 20,
      'sessions_per_slot', v_wave_size,
      'work_window_central', '05:15-16:00',
      'assignment_source', 'daily_schedule_assignments + location_group_memberships',
      'last_slot', v_slot,
      'last_started_count', v_started
    )
  where id = v_run_id;
  ...
  return jsonb_build_object(
    ...
    'slot', v_slot,
    'started', v_started,
    'desired_type', v_desired_type,
    'closed_sessions', v_closed,
    'sessions', v_started_details
  );
end $$;
